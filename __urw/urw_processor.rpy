...
init -997 python:
    ...
    class URWMenuWrapper:
        ...
        def _enhance_item(self, item, index, menu_node, match_info):
            ...
            try:
                ...
                c_size = getattr(persistent, 'urw_choice_text_size', None)
                if c_size is not None and c_size > 0:
                    caption = f"{{size={c_size}}}{caption}{{/size}}"
                ...
                if new_caption != str(raw_caption):
                    if is_object_style:
                        try:
    ...
    def urw_apply_button_adaptation():
        """Modify choice_button style to auto-adapt to text height"""
        try:
            if "choice_button" in style:
                style.choice_button.ysize = None
                style.choice_button.yminimum = 45
                renpy.style.rebuild()
                urw_log.info("Applied choice_button auto-adaptation", "INIT")
        except Exception as e:
            urw_log.error(f"Failed to adapt choice buttons: {e}", "INIT")
